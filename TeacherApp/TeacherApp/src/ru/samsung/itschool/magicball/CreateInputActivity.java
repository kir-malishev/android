...
public class CreateInputActivity extends Activity {
	...
	public void saveqq() {
		...
		editor.putInt(login + "_size_" + position, editTextList.size());
		for (int i = 0; i < editTextList.size(); i++) {
			editor.putString(login + "_ans_" + position + "_" + i, editTextList.get(i).getText().toString());
		}
		editor.commit();
	}
	...
	public void getqq() {
		...
		for (int i = 0; i < size; i++) {
			ans = sharedPref.getString(login + "_ans_" + position + "_" + i, "");
			if (ans.equals("") == false) {
				addEdit();
	...
	public void addEdit() {
		...
		if (size <= 7) {
			...
			editTxt.setHint("Вариант №" + (size + 1));
